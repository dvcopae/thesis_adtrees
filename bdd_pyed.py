...
SU, DNS, ACV, ESV, APUT, PA, BU, SKO, SDK = map(bddvar, 'SU DNS ACV ESV APUT PA BU SKO SDK'.split())

formula_expr = ((~(SU & ~DNS) & ACV) | (~(SU & ~DNS) & ESV) | (~APUT & PA) | BU) & (~SKO & SDK)
...
for sat_raw in formula_bdd.satisfy_all():
    ...
    def_cost = sum([ba[d] for d in T.get_basic_actions('d') if d in sat and sat[d] == 1])
    att_cost = sum([ba[a] for a in T.get_basic_actions('a') if a in sat and sat[a] == 1])
